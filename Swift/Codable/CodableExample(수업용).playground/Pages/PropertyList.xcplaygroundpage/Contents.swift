...
/*:
 ...
 ---
 */
let macArr = [macBook, macBook, macBook]
let macDict = ["mac": macBook, "mac2": macBook, "mac3": macBook ]

//Encoding
func encodingMacs(arr: [MacBook], dict: [String: MacBook]) -> (arr: Data, dict: Data)? {
    let encoder = PropertyListEncoder()
    guard
        let encodedArr = try? encoder.encode(arr),
        let encodedDict = try? encoder.encode(dict)
        else { return nil }
    
    return (encodedArr, encodedDict)
}
let decodDatas = encodingMacs(arr: macArr, dict: macDict)

func decodingMac(arrData: Data, dictData: Data) -> (arr: [MacBook], dict: [String: MacBook])? {

    let decoder = PropertyListDecoder()
    guard
        let arr = try? decoder.decode([MacBook].self, from: arrData),
        let dict = try? decoder.decode([String: MacBook].self, from: dictData)
        else { return nil}
    
    return (arr, dict)
}

if let datas = decodDatas {
    print(datas)
    let macs = decodingMac(arrData: datas.arr, dictData: datas.dict)
    print(macs ?? "Nil")
}
