...
/*:
 ...
 ---
 */
var forEachArray: [Int] = []
for i in 1...10 {
    forEachArray.append(i)
}
...
// 리턴하는 불값이 true 이면 반환하는 컬렉션에 넣고 false이면 반환하는 컬렉션에 넣지 않는다.
...
let reduceString = ["1", "2", "3"].reduce("", {
    $0 + $1
})

print(reduceString)

let reduceInt = [1, 2, 3].reduce("", {(result: String, nextParam: Int ) -> String in
    return result + String(nextParam)
})
