...
//    user.friends.append(friend)
    ...
    // 최종적으로 user.friends 에 들어있는 friend 의 숫자는? -> 1명
  ...
  func blockFriend(name: String) {
    ...
    if let index = User.shared.friends.firstIndex(of: friend) {// 친구목록에 있는지 보고 있으면 삭제
      User.shared.friends.remove(at: index)
    }
    if !User.shared.blocks.contains(friend) { // 기존에 있으면 추가 하지 않고
