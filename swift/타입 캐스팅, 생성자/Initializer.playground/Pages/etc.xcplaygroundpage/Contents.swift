...
class Chart {
  ...
  init(height: Int, xPosition: Int) {
    self.height = height
    print(self.height)    // 초기화 이후 -> 접근 가능
//    print(self.xPosition) // 초기화 이전 -> 접근 불가
    
//    self.printValues()     // 저장 프로퍼티 전체 초기화 이전 -> 에러 -> 모든 저장 프로퍼티가 초기화 되어있어야 함
    self.xPosition = xPosition
    print(self.height, self.xPosition) // 초기화 이후 -> 접근 가능
//    self.printValues()     // 저장 프로퍼티 전체 초기화 이후 -> 메서드 호출 가능
  }
...
class Animal {
  ...
  required init(name: String) {// 자기 자신에서는 어떤 생성자를 쓰던 상관없음
    self.name = name
    age = 3
  }
}
...
someClass = SomeClass()
// 다른 인스턴스로 바꿔주면 기존 인스턴스가 삭제 된다

var someClass2: SomeClass? = SomeClass()
someClass2 = nil
// nil 값을 넣어주면 인스턴스 삭제
